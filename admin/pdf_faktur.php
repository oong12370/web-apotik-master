...
$pdf->setFont('Arial','B',14);
$pdf->setXY(90,34);
$pdf->Cell(40,10,'Nota Penjualan Obat ');
//Latar
...
$pdf->Cell(11,30,': '.$data['no_transaksi'],0,1, 'L');
$pdf->setXY(24,40);
$pdf->Cell(30,30,'Nama ');
$pdf->Cell(40,30,': '.$data['nm_konsumen'],0,1, 'L');
$pdf->setXY(24,44);
$pdf->Cell(30,30,'Keluhan ');
$pdf->Cell(40,30,': '.$data['keluhan'],0,1, 'L');
...
$query = "SELECT  tbl_transaksi.no_transaksi, tbl_transaksi.tgl, tbl_transaksi.subtotal,tbl_transaksi.kd_admin, tbl_konsumen.kd_konsumen,tbl_konsumen.jekel, tbl_konsumen.nm_konsumen,tbl_konsumen.keluhan, detail.no_transaksi,tbl_admin.nm_admin,detail.jumlah,detail.harga,detail.nm_katagori,  tbl_obat.kd_obat, tbl_obat.nm_obat from detail
...
$harga=$data['harga'];
$total=$harga*$jumlah;
$sql2=mysql_query("SELECT sum(harga*jumlah) as jml FROM detail where no_transaksi='$no_transaksi'");
...
$pdf->setY($y);
	...
	$pdf->Cell(30,6,$data['nm_katagori'],1,0,'L');
	...
	$pdf->Cell(40,6,$rs2['jml'],1,0,'L');
	$y = $y + $row;
$sql=mysql_query("SELECT  tbl_transaksi.no_transaksi, tbl_transaksi.tgl, tbl_transaksi.subtotal,tbl_transaksi.kd_admin, tbl_konsumen.kd_konsumen,tbl_konsumen.jekel, tbl_konsumen.nm_konsumen,tbl_konsumen.keluhan, detail.no_transaksi,tbl_admin.nm_admin,detail.jumlah,detail.harga,detail.nm_katagori,  tbl_obat.kd_obat, tbl_obat.nm_obat from detail
  inner join tbl_transaksi on  detail.no_transaksi = tbl_transaksi.no_transaksi
  inner join tbl_konsumen on tbl_transaksi.kd_konsumen = tbl_konsumen.kd_konsumen
  inner join tbl_admin on tbl_transaksi.kd_admin = tbl_admin.kd_admin
  inner join tbl_obat on detail.kd_obat = tbl_obat.kd_obat
  WHERE tbl_transaksi.no_transaksi like '%$cari_data%'");//order by
$data=mysql_fetch_array($sql);
	$pdf->setY($y);
	$pdf->setXY(25,$y);
	$pdf->Cell(30,30,'Order by ');
